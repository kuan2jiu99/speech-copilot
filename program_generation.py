...
def prompt_template(instruction, task_name):

    def get_dataset_list():
        dataset_list_path = "./prompt_templates/dataset_list.txt"
        dataset_list = []

        with open(dataset_list_path, "r") as file:
            for line in file:
                dataset_list.append(f"DynamicSuperb/{line.strip()}")

        return dataset_list  

    assert task_name in get_dataset_list(), f"Task name {task_name} not found in the dataset list."

    if task_name in ["SpokenTermDetection_LJSpeech", "SpokenTermDetection_LibriSpeech-TestClean", "SpokenTermDetection_LibriSpeech-TestOther"]:
        return prompt_template_with_word(instruction)
    elif task_name in ["SpeechTextMatching_LJSpeech", "SpeechTextMatching_LibriSpeech-TestClean", "SpeechTextMatching_LibriSpeech-TestOther"]:
        return prompt_template_with_text(instruction)
    elif task_name in ["DialogueActPairing_DailyTalk", "SpeakerVerification_LibriSpeech-TestClean", "SpeakerVerification_VCTK"]:
        return prompt_template_two_audio(instruction)
    else:
        return prompt_template_usual(instruction)
...
def main(args):
    ...
    user_prompt = prompt_template(args.instruction, task_name=args.task_name)
    print(f"User Prompt:\n {user_prompt}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_name", type=str, help="The task name.", default="DynamicSuperb/EmotionRecognition_MultimodalEmotionlinesDataset")
    ...
    # Decomposed speech modules generated by LLM.
    parser.add_argument("--speech_module_template_path", type=str, help="The path to the decomposed speech modules generated by LLM.", default="./speech_module_generated_by_LLM.py")

    # Pivotal LLM model name.
